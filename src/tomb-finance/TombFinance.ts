...
/**
 * An API module of Comet Finance contracts.
...
export class TombFinance {
  ...
  constructor(cfg: Configuration) {
    ...
    this.COMET = new ERC20(deployments.tomb.address, provider, 'COMET');
    this.METEOR = new ERC20(deployments.tShare.address, provider, 'METEOR');
    this.CBOND = new ERC20(deployments.tBond.address, provider, 'CBOND');
    this.FTM = this.externalTokens['WFTM'];

    // Uniswap V2 Pair
    this.COMETWFTM_LP = new Contract(externalTokens['COMET-FTM-LP'][0], IUniswapV2PairABI, provider);
  ...
  unlockWallet(provider: any, account: string) {
    ...
    const tokens = [this.COMET, this.METEOR, this.CBOND, ...Object.values(this.externalTokens)];
    for (const token of tokens) {
      token.connect(this.signer);
    }
    this.COMETWFTM_LP = this.COMETWFTM_LP.connect(this.signer);
  ...
  async getTombStat(): Promise<TokenStat> {
    const { CometFtmRewardPool } = this.contracts;
    const supply = await this.COMET.totalSupply();
    const tombRewardPoolSupply = await this.COMET.balanceOf(CometFtmRewardPool.address);
    // const tombRewardPoolSupply2 = await this.COMET.balanceOf(CometFtmLPCometRewardPool.address);
    // const tombRewardPoolSupplyOld = await this.COMET.balanceOf(CometFtmLPCometRewardPoolOld.address);
    const tombCirculatingSupply = supply.sub(tombRewardPoolSupply);
      // .sub(tombRewardPoolSupply2);
      // .sub(tombRewardPoolSupplyOld);
    const priceInFTM = await this.getTokenPriceFromPancakeswap(this.COMET);
    ...
    return {
      ...
      totalSupply: getDisplayBalance(supply, this.COMET.decimal, 0),
      circulatingSupply: getDisplayBalance(tombCirculatingSupply, this.COMET.decimal, 0),
    };
  }
  ...
  async getLPStat(name: string): Promise<LPStat> {
    ...
    const token0 = name.startsWith('COMET') ? this.COMET : this.METEOR;
    const isTomb = name.startsWith('COMET');
  ...
  async getShareStat(): Promise<TokenStat> {
    const { CometFtmLPMeteorRewardPool } = this.contracts;

    const supply = await this.METEOR.totalSupply();

    const priceInFTM = await this.getTokenPriceFromPancakeswap(this.METEOR);
    const tombRewardPoolSupply = await this.METEOR.balanceOf(CometFtmLPMeteorRewardPool.address);
    ...
    return {
      ...
      totalSupply: getDisplayBalance(supply, this.METEOR.decimal, 0),
      circulatingSupply: getDisplayBalance(tShareCirculatingSupply, this.METEOR.decimal, 0),
    };
  }

  async getTombStatInEstimatedTWAP(): Promise<TokenStat> {
    const { SeigniorageOracle, CometFtmRewardPool } = this.contracts;
    const expectedPrice = await SeigniorageOracle.twap(this.COMET.address, ethers.utils.parseEther('1'));
  ...
  async getCometPriceInLastTWAP(): Promise<BigNumber> {
    const { Treasury } = this.contracts;
    return Treasury.getCometUpdatedPrice();
  }

  async getBondsPurchasable(): Promise<BigNumber> {
    const { Treasury } = this.contracts;
    return Treasury.getBurnableCometLeft();
  }
  ...
  async getPoolAPRs(bank: Bank): Promise<PoolStats> {
    ...
    const stat = bank.earnTokenName === 'COMET' ? await this.getTombStat() : await this.getShareStat();
  ...
  async getTokenPerSecond(
    ...
    if (earnTokenName === 'COMET') {
      if (!contractName.endsWith('CometRewardPool')) {
        const rewardPerSecond = await poolContract.cometPerSecond();
        if (depositTokenName === 'WFTM') {
          return rewardPerSecond.mul(7000).div(20000);
      ...
      if (Date.now() - startDateTime.getTime() > FOUR_DAYS) {
        return await poolContract.epochCometPerSecond(1);
      }
      return await poolContract.epochCometPerSecond(0);
    }
    const rewardPerSecond = await poolContract.MeteorPerSecond();
    if (depositTokenName.startsWith('COMET')) {
      return rewardPerSecond.mul(39000).div(65000);
    } else {
      return rewardPerSecond.mul(26000).div(65000);
    }
  }
  ...
  async getDepositTokenPriceInDollars(tokenName: string, token: ERC20) {
    ...
    if (tokenName === 'WFTM') {
      ...
      if (tokenName === 'COMET-FTM-LP') {
        tokenPrice = await this.getLPTokenPrice(token, this.COMET, true);
      } else if (tokenName === 'METEOR-FTM-LP') {
        tokenPrice = await this.getLPTokenPrice(token, this.METEOR, false);
  ...
  async buyBonds(amount: string | number): Promise<TransactionResponse> {
    const { Treasury } = this.contracts;
    const treasuryTombPrice = await Treasury.getCometPrice();
  ...
  async redeemBonds(amount: string): Promise<TransactionResponse> {
    const { Treasury } = this.contracts;
    const priceForTomb = await Treasury.getCometPrice();
  ...
  async getTotalValueLocked(): Promise<Number> {
    ...
    const masonrytShareBalanceOf = await this.METEOR.balanceOf(this.currentMasonry().address);
    const masonryTVL = Number(getDisplayBalance(masonrytShareBalanceOf, this.METEOR.decimal)) * Number(TSHAREPrice);
  ...
  async earnedFromBank(
    ...
    try {
      if (earnTokenName === 'COMET') {
        return await pool.pendingCOMET(poolId, account);
      } else {
  ...
  async getMasonryAPR() {
    ...
    const masonrytShareBalanceOf = await this.METEOR.balanceOf(Masonry.address);
    const masonryTVL = Number(getDisplayBalance(masonrytShareBalanceOf, this.METEOR.decimal)) * Number(TSHAREPrice);
  ...
  async canUserUnstakeFromMasonry(): Promise<boolean> {
    ...
    const notStaked = Number(getDisplayBalance(stakedAmount, this.METEOR.decimal)) === 0;
  ...
  async getGenesisPoolStartAndEndTime(): Promise<AllocationTime> {
    const { CometFtmRewardPool } = this.contracts;
    const startTimestamp: BigNumber = await CometFtmRewardPool.poolStartTime();
    const endTimestamp: BigNumber = await CometFtmRewardPool.poolEndTime();
  ...
  async getMeteorPoolStartAndEndTime(): Promise<AllocationTime> {
    const { MeteorFtmLPMeteorRewardPool } = this.contracts;
    const startTimestamp: BigNumber = await MeteorFtmLPMeteorRewardPool.poolStartTime();
    const endTimestamp: BigNumber = await MeteorFtmLPMeteorRewardPool.poolEndTime();
    const startAllocation = new Date(startTimestamp.mul(1000).toNumber());
    const endAllocation = new Date(endTimestamp.mul(1000).toNumber());
  ...
  async watchAssetInMetamask(assetName: string): Promise<boolean> {
    ...
    if (ethereum && ethereum.networkVersion === config.chainId.toString()) {
      ...
      if (assetName === 'COMET') {
        asset = this.COMET;
        assetUrl = 'https://raw.githubusercontent.com/krypto-dev/comet-finance-assets/main/crypto_tomb_cash-svg.png';
      } else if (assetName === 'METEOR') {
        asset = this.METEOR;
        assetUrl = 'https://raw.githubusercontent.com/krypto-dev/comet-finance-assets/main/crypto_tomb_share-svg.png';
      } else if (assetName === 'CBOND') {
        asset = this.CBOND;
        assetUrl = 'https://raw.githubusercontent.com/krypto-dev/comet-finance-assets/main/crypto_tomb_bond-svg.png';
      }
  ...
  async quoteFromSpooky(tokenAmount: string, tokenName: string): Promise<string> {
    ...
    const { _reserve0, _reserve1 } = await this.COMETWFTM_LP.getReserves();
    let quote;
    if (tokenName === 'COMET') {
  ...
  async estimateZapIn(tokenName: string, lpName: string, amount: string): Promise<number[]> {
    ...
    if (tokenName === FTM_TICKER) {
      ...
      const token = tokenName === TOMB_TICKER ? this.COMET : this.METEOR;
  ...
  async swapTBondToTShare(tbondAmount: BigNumber): Promise<TransactionResponse> {
    ...
    return await TShareSwapper.swapCBondToMeteor(tbondAmount);
  }
  async estimateAmountOfMeteor(tbondAmount: string): Promise<string> {
    const { TShareSwapper } = this.contracts;
    try {
      const estimateBN = await TShareSwapper.estimateAmountOfMeteor(parseUnits(tbondAmount, 18));
      return getDisplayBalance(estimateBN, 18, 6);
    } catch (err) {
      console.error(`Failed to fetch estimate meteor amount: ${err}`);
    }
  }

  async getTShareSwapperStat(address: string): Promise<TShareSwapperStat> {
    const { TShareSwapper } = this.contracts;
    const tshareBalanceBN = await TShareSwapper.getMeteorBalance();
    const tbondBalanceBN = await TShareSwapper.getCBondBalance(address);
    // const tombPriceBN = await TShareSwapper.getCometPrice();
    // const tsharePriceBN = await TShareSwapper.getTSharePrice();
    const rateTSharePerTombBN = await TShareSwapper.getMeteprAmountPerComet();
