...
const Sbs: React.FC = () => {
  ...
  const [approveStatus, approve] = useApprove(tombFinance.CBOND, tombFinance.contracts.TShareSwapper.address);
  ...
  const handleTBondChange = async (e: any) => {
    ...
    const updateTShareAmount = await tombFinance.estimateAmountOfMeteor(e.currentTarget.value);
    setTshareAmount(updateTShareAmount);  
  };

  const handleTBondSelectMax = async () => {
    setTbondAmount(String(bondBalance));
    const updateTShareAmount = await tombFinance.estimateAmountOfMeteor(String(bondBalance));
    setTshareAmount(updateTShareAmount); 
  };
        ...
        {!!account ? (
          <>
            <Route exact path={path}>
              <PageHeader icon={'🏦'} title="CBond -> Meteor Swap" subtitle="Swap CBond to Meteor" />
            </Route>
            <Box mt={5}>
                  ...
                  <StyledCardsWrapper>
                        ...
                        <CardContent>
                          <StyledCardContentInner>
                            ...
                            <StyledExchanger>
                              <StyledToken>
                                <StyledCardIcon>
                                  <TokenSymbol symbol={tombFinance.CBOND.symbol} size={54} />
                                </StyledCardIcon>
                              </StyledToken>
                            </StyledExchanger>
                            <Grid item xs={12}>
                    ...
                    <StyledCardWrapper>
                      <Card>
                        <CardContent>
                          <StyledCardContentInner>
                            <StyledCardTitle>Meteor</StyledCardTitle>
                            <StyledExchanger>
                              <StyledToken>
                                <StyledCardIcon>
                                  <TokenSymbol symbol={tombFinance.METEOR.symbol} size={54} />
                                </StyledCardIcon>
                              </StyledToken>
                            </StyledExchanger>
                            <Grid item xs={12}>
                            ...
                            <StyledDesc>{`${tshareBalance} METEOR Available in Swapper`}</StyledDesc>
                          </StyledCardContentInner>
                        </CardContent>
                      </Card>
