              ...
              <Box mt={5}>
                <div hidden={activeBanks.filter((bank) => bank.sectionInUI === 2).length === 0}>
                  <Typography color="textPrimary" variant="h4" gutterBottom>
                    Earn METEOR by staking LP
                  </Typography>
                  <Alert variant="filled" style={{background: '#b648bd'}}>
                    {isStart ? 
                      <div>Pools are live now, Stake LPs to earn more METEOR, No deposit fee</div> : 
                      <>
                        Pools starting at {mfrom.toUTCString()}, No deposit fee.<br/>
                        <div style={{display:'flex'}}>Meteor reward pools start in: <ProgressCountdown base={moment().toDate()} hideBar={true} deadline={mfrom} description="End Pool" />.</div>
                      </>
                    }
                  </Alert>
                  ...
                  <Alert variant="filled" severity={isOver ? 'info' : 'success'} style={{background: '#b648bd'}}>
                    {isOver ? 
                      ...
                      <>
                        ...
                        <div>Please refer to <a target="_blank" href="https://comet-finance88.gitbook.io/untitled/" rel="noopener noreferrer">documentation</a> docs to understand our protocol's fee model.</div>
                      </>
                    }
                  </Alert>
                  <Grid container spacing={3} style={{ marginTop: '20px' }}>
