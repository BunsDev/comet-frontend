...
const useEstimateTShare = (tbondAmount: string) => {
  ...
  const estimateAmountOfTShare = useCallback(async () => {
    ...
    const amount = await tombFinance.estimateAmountOfMeteor(tbondAmountBn.toString());
    setEstimateAmount(amount);
  }, [account]);

  useEffect(() => {
    if (account) {
      estimateAmountOfTShare().catch((err) => console.error(`Failed to get estimateAmountOfMeteor: ${err.stack}`));
    }
